...
ID = answers["ID"]
PW = getpass("[SECRET]: (보안 모드)비밀번호를 입력해주세요:")
print(f"[INIT]: 설정 로드 완료 - {NUMBER_OF_IMAGES}장 씩 가져옵니다.")
...
for target in mode_set["target"]:
    ...
    for link in links:
        browser.get(link)
        img_cont = browser.find_element_by_xpath(
            '//*[@id="react-root"]/section/main/div/div[1]/article/div[2]'
        )
        ...
        cnt += 1
print("[COMPLETE]: 🚀All processes are completed. ")
browser.quit()
sys.exit()
# download the image
